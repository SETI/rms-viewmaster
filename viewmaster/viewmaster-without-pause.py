...
def get_holdings_paths_old_way():
    ...
    holdings_abspaths = []
    for diskname in os.listdir('/Volumes'):
        match = DISKNAME_REGEX.match(diskname)
        if not match: continue

        holdings_abspaths.append('/Volumes/' + diskname + '/holdings')
...
def create_holdings_symlinks(abspaths):
    ...
    for k, abspath in enumerate(abspaths):
        symlink = '/Library/WebServer/Documents/holdings' + \
                  (str(k) if k else '')
    ...
    for k in range(len(abspaths), 10):
        symlink = '/Library/WebServer/Documents/holdings' + str(k)
...
def load_infopage_content(page_pdsfile, hrefs=True):
    ...
    for (recno, basename, abspath) in link_info:
        ...
        if ext in VIEWABLE_EXTENSIONS:
            href = (f'<a href="{LOCALHOST_}{pdsf.url}" target="_blank">' +
...
SAFE_FILTER_REGEX = re.compile('^\w+\*(|\.*)$', re.I)
...
def get_product_page_info(query_pdsfile):
    ...
    # Identify all local files sharing the same anchor
    if query_pdsfile.is_index_row:
    ...
    # Find the info file if any
    info_pdsfile = None
    if query_pdsfile.extension.lower() in ('.cat', '.txt', '.fmt'):
    ...
    # Insert an empty table in front for an index row
    if query_pdsfile.is_index_row:
    ...
    # Next in the table is this object of the index if this is an index row
    table1 = PdsGroupTable()
    table1.insert_group(PdsGroup(sib_pdsfile))  # Query file is first
    for pdsf in siblings:                       # Query siblings are ungrouped
        if pdsf.logical_path != sib_pdsfile.logical_path:
...
def viewmaster(query_path):
    ...
    try:
        ...
        if query_pdsfile.logical_path != query_path:
            ...
            elapsed = delta.seconds + delta.microseconds * 1.e-6
